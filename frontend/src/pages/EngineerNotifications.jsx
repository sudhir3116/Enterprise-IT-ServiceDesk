...
export default function EngineerNotifications() {
  ...
  // Filter state: 'all' | 'assignment' | 'sla' | 'update' | 'note' | 'announcement'
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      // Update local state to read: true
  ...
  // Helper to resolve icon based on message title / keywords
  const getNotificationIcon = (title = '', message = '') => {
    ...
    if (t.includes('sla') || t.includes('due') || t.includes('breach')) {
      return <AlertTriangle size={16} className="text-red-500" />
    }
    if (t.includes('comment') || t.includes('note') || t.includes('reply') || t.includes('mention')) {
  ...
  // Keyword filter categorization logic
  const filteredNotifs = notifications.filter(n => {
    if (filterType === 'all') return true
    const t = (n.title + ' ' + n.message).toLowerCase()
    
    if (filterType === 'assignment') {
      return t.includes('assign') || t.includes('allocat')
    }
    if (filterType === 'sla') {
      return t.includes('sla') || t.includes('due') || t.includes('breach')
    }
    if (filterType === 'update') {
      return t.includes('status') || t.includes('update')
    }
    if (filterType === 'note') {
      return t.includes('comment') || t.includes('note') || t.includes('reply') || t.includes('mention')
    }
    ...
    return (
      <div className="flex justify-center items-center h-[70vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-[var(--ds-text-muted)]">Loading Notification Inbox…</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-10">
      ...
      <PageHeader
        ...
        description="Manage your incident allocations, SLA triggers, mentions, and system updates."
        icon={Bell}
        breadcrumbs={[{ name: 'Engineer', path: '/engineer/dashboard' }, { name: 'Inbox' }]}
        primaryAction={
          unreadCount > 0 ? (
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card className="p-4 space-y-2">
            ...
            {[
              ...
              { id: 'assignment', label: 'Assignments', count: notifications.filter(n => (n.title+n.message).toLowerCase().includes('assign') || (n.title+n.message).toLowerCase().includes('allocat')).length },
              { id: 'sla', label: 'SLA Alerts', count: notifications.filter(n => (n.title+n.message).toLowerCase().includes('sla') || (n.title+n.message).toLowerCase().includes('due') || (n.title+n.message).toLowerCase().includes('breach')).length },
              { id: 'update', label: 'Ticket Updates', count: notifications.filter(n => (n.title+n.message).toLowerCase().includes('status') || (n.title+n.message).toLowerCase().includes('update')).length },
              { id: 'note', label: 'Notes & Replies', count: notifications.filter(n => (n.title+n.message).toLowerCase().includes('comment') || (n.title+n.message).toLowerCase().includes('note') || (n.title+n.message).toLowerCase().includes('reply')).length },
              { id: 'announcement', label: 'Announcements', count: notifications.filter(n => (n.title+n.message).toLowerCase().includes('broadcast') || (n.title+n.message).toLowerCase().includes('announc')).length },
            ].map(type => (
              <button
                key={type.id}
                    ...
                    >
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {targetTicketId && (
                            <Link 
                              to={`/engineer/ticket/${targetTicketId}`}
                            ...
                            >
                              <Eye size={11} /> View Ticket Details
                            </Link>
                          )}
                        </div>
                      </div>

                      {/* Triage Mark as Read button */}
                      {!n.read && (
                        <button
                          onClick={() => handleMarkSingleRead(n._id)}
                          title="Mark as Read"
